...
spOK = krige.conv(gd, locations=gridOrd,
           krige = krige.control(
...
expImageOK = list(x = D, y = D, z = predOK)
VarImageOK = list(x = D, y = D, z = varOK)
# par(mfrow=(c(1,2)))

# Kriging predictor
# pdf("2c_ordKrigPred.pdf")
image.plot(expImageOK, col=terrain.colors(200),asp=1)
contour(expImageOK, add = T)
# dev.off()
...
# par(mfrow=(c(1,1)))
```
```{r 2c_3d, include=F, eval=F}
grid2c = grid
grid2c$z = expImageOK
# surface3d = s3dPlot(expImageOK, fn = "2c_ordKrigSurface.pdf")
```
...
predDiff = abs(predUK - predOK)
breaksPred = 1.1^(-190:10)/1.1^10*max(predDiff)
pdf("2d_predDiff.pdf")
image.plot(list(x=D,y=D,z=predDiff), col=viridis(200), asp=1,breaks = breaksPred)
# points(dt$x,dt$y, pch=4, col = "white")
dev.off()

sigmaDiff = varUK - varOK
min(sigmaDiff)
breaksPred = 1.1^(-190:10)/1.1^10*max(sigmaDiff)
pdf("2d_varDiff.pdf")
image.plot(list(x=D,y=D,z=sigmaDiff), col=viridis(200), asp=1, breaks = breaksVar)
# points(dt$x,dt$y, pch=4, col = "white")
dev.off()
```
...
predUK = matrix(spUK$predict,ncol = 315)
imageUKe = list(x=D,y=D,z=predUK)
# # pdf("2e_ordKrigPred.pdf")
# # image.plot(D,D,z=predUK, col=terrain.colors(200), asp=1)
# # contour(D,D,predUK, add = T)
# # points(dt$x,dt$y, col="red", pch=4)
# dev.off()
# 
# # Associated prediction variance
# sigmaHat = matrix(spUK$krige.var,ncol = 315)
# pdf("2e_ordKrigVar.pdf")
# image.plot(x=D,y=D,z=sigmaHat, col=viridis(200), asp=1)
# points(gd[[1]][,1],gd[[1]][,2], pch=4, col = "white")
# dev.off()
...
We have assumed normally distributed errors $\varepsilon$ for the linear regression model. Thus, our predictions $\hat X(s_o) = \hat X((100,100)^T) =$ are normally distributed about the mean $E\hat X_0$. Specifically for $s_0 = (100,100)^T$ we have $\hat X((100,100)^T) =$
